...
void shut_down() {
  ...
  SSL_write(ssl_client,buffer,sizeof(buffer));
  if (log_flag)
...
SSL_CTX * ssl_init(void) {
  ...
  if (newContext == NULL) {
    fprintf(stderr, "Could not create SSL_OBJ\n");
    exit(2);
  }
...
SSL * attach_ssl_to_socket(int socket, SSL_CTX * context) {
  ...
  if (sslClient == NULL) {
    fprintf(stderr, "Creation of new SSL structure failed\n");
    exit(2);
  }
  
  if (SSL_set_fd(sslClient, socket) != 1) {
    fprintf(stderr, "Connect the SSL object with a file descriptor fails\n");
    exit(2);
  }
  
  if (SSL_connect(sslClient) < 0) {
    fprintf(stderr, "Initiate the TLS/SSL handshake with an TLS/SSL server fails\n");
    exit(2);
  }
  return sslClient;
}
...
int main(int argc, char * argv[]) {
  ...
  ssl_client = attach_ssl_to_socket(client_socket, context);


  SSL_write(ssl_client,id,sizeof(id));
  ...
  while (1) {
	...
	char buf[256];
	int ret = SSL_read(ssl_client, buf, sizeof(buf));
	if (ret < 0) {
	...
	temp = temp * 9/5 + 32;
      ...
      SSL_write(ssl_client,buf2,sizeof(buf2));
